...
Die Struktur der Arbeit erlaubt unterschiedliche Formen der Annäherung. Neben der sequenziellen Lektüre bietet sich ein Zugang an, der bei den Ergebnissen einsetzt und von dort aus schrittweise Bezüge herstellt. Abbildung~\ref{fig:lesepfad-entdeckend} visualisiert diesen explorativen Lesepfad als systemische Schleife und macht die rekursive Anlage der Untersuchung als Bewegung nachvollziehbar. Als Orientierung dienen insbesondere \hyperref[sec:Ergebnisse-Forschungsfragen]{Befund (Abschnitt~\ref*{sec:Ergebnisse-Forschungsfragen})}, \hyperref[sec:Entwicklung-Einbettung]{Kontext (Abschnitt~\ref*{sec:Entwicklung-Einbettung})}, \hyperref[sec:Regeneration-Stoerung]{Linse (Abschnitt~\ref*{sec:Regeneration-Stoerung})}, \hyperref[sec:Datenanalyse]{Prüfung (Abschnitt~\ref*{sec:Datenanalyse})}, \hyperref[sec:Diskussion-Interdependenz]{Verdichtung (Abschnitt~\ref*{sec:Diskussion-Interdependenz})} sowie \hyperref[sec:Conclusio-Manifest]{Abschluss (Abschnitt~\ref*{sec:Conclusio-Manifest})}. Der Zugang kann an jeder Station beginnen und rekursiv über Befund, Kontext, Theorie, Prüfung und Verdichtung geführt werden; bei Orientierungsbedarf kann Kapitel 1 als Kompass hinzugezogen werden.

```{=latex}
\input{08 Metaquellen/08-01 Abbildungen/prozesse/lesepfad-entdeckend.tex}
```

Dieser explorative Lesepfad ist als Einladung zu einem vertieften und flexiblen Lesen gedacht und ergänzt den operationalen Lesepfad. Wer so einsteigt, kann die jeweiligen Kapitel entlang des operationalen Lesepfads vertiefen und die Begründungsschritte vollständig nachvollziehen.
