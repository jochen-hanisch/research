...
- \hyperref[sec:Datenanalyse]{Abschnitt 4.3}: Datenanalyse
- \hyperref[sec:Simulation-Kompetenzentwicklung]{Abschnitt 4.4}: simulationsgestützte Modellierung
- \hyperref[sec:Methoden-Reflexion]{Abschnitt 4.5}: methodische Reflexion
...
Für das vorliegende Forschungsvorhaben steht nicht die Zuordnung zu einem einzelnen methodologischen Paradigma im Vordergrund, sondern die Frage, wie sich die Forschungsunterfragen gegenstandsangemessen bearbeiten lassen. Gewählt wird deshalb ein forschungsfragengeleiteter, systemisch integrierender Zugriff, der qualitative, quantitative und simulationsbasierte Verfahren über die Logik der Forschungsunterfragen miteinander verbindet. Ob diese Logik trägt, wird im Kapitelverlauf eingelöst:
...
Die Forschungsfragen bestimmten die Auswahl und Strukturierung der Literatur, die Entwicklung von Kategorien und Schlagworten zur thematischen Verknüpfung sowie die Kombination und Anpassung klassischer Methoden. Die Komplexität des digitalen Bildungsraums machte diese Methodenkombination erforderlich, damit die Forschungsfragen in einer dem Gegenstand angemessenen Form bearbeitet werden konnten.
...
Zur Vorbereitung der Datenanalyse wurde in Zotero eine gestufte Suchordnerstruktur mit zwölf priorisierten Ordnern (`0` bis `b`) angelegt. Jeder Ordner bündelt einen Eintragstyp mit einer klar definierten Schlagwortlogik. Die Treffer wurden in einer festen Reihenfolge gesichtet und beim ersten passenden Treffer eindeutig markiert. Auf diese Weise ließ sich der Suchraum systematisch erweitern, ohne Dubletten oder semantische Überschneidungen unnötig zu vervielfachen. Die Grundstruktur der Suchordner ist in \hyperref[tab:primaere_suchbegriffe]{Tabelle~\ref{tab:primaere_suchbegriffe}}, \hyperref[tab:sekundaere_suchbegriffe]{Tabelle~\ref{tab:sekundaere_suchbegriffe}} und \hyperref[tab:tertiäre_suchbegriffe]{Tabelle~\ref{tab:tertiäre_suchbegriffe}} dokumentiert [@doring_forschungsmethoden_2023, Kapitel 10.6; @galvan_writing_2025].

Die vollständige Ordnerstruktur in Zotero ist in \hyperref[sec:A-6]{Anhang „Struktur der Suchordner“} dokumentiert.

Die Suchordnerlogik ist entlang primärer, sekundärer und tertiärer Suchbegriffe aufgebaut. Sie bildet die thematische Nähe zum Forschungsgegenstand ab und steuert zugleich die Reihenfolge der Sichtung. Die Ordnerkennungen (z. B. `S:01`) entsprechen der Zotero-Struktur und wurden zugleich als Tags verwendet, sodass Suchpfad und spätere Auswertung durchgehend aufeinander bezogen bleiben.
...
Die Abbildung zeigt die Suchordnerstrategie in ihrer konkreten technischen Umsetzung. Im Beispiel wird eine Bool’sche Suchdefinition für Zeitschriftenartikel im Schnittfeld von `learning`, `management` und `system` mit der deutschsprachigen Variante „Lernmanagementsystem“ kombiniert. Ergänzt wird dies durch negative Tags (z. B. `Promotion:Ausschluss`, `#2–#b`) sowie den Ausschluss übergeordneter Sammlungen (z. B. `S:01`). Auf diese Weise bleiben thematisch passende, begutachtete Fachbeiträge im Suchraum, während Dubletten, bereits ausgeschlossene Titel und randständige Treffer früh ausgefiltert werden. Methodisch entspricht dies einer qualitativ-kriterialen Dokumentenselektion, deren Regeln transparent dokumentiert und im Workflow reproduzierbar gehalten werden [@doring_forschungsmethoden_2023, Kapitel 10.6].

Die Suchordnerstrategie folgt einer festen Schrittlogik. Zunächst wird ein Suchpfad gewählt, der den semantischen Abstand zum Forschungsgegenstand markiert: primäre Ordner für den direkten LMS-Bezug, sekundäre Ordner für kontextualisierende bildungstechnologische Nachbarschaften und tertiäre Ordner für periphere Trend- und Randfeldsuche. Anschließend wird der jeweilige Ordner technisch über Eintragstyp, Bool’sche Begriffskombinationen und Sprachvarianten definiert. Erst dann erfolgt die Sichtung der Trefferliste. Dass die Sichtungsquote mit wachsender Distanz zum Kerngegenstand sinkt, ist methodisch bewusst gesetzt: Der Suchraum bleibt offen genug für relevante Anschlüsse, ohne dass das analytische Zentrum durch randständige Literatur verwässert wird. Relevante Titel werden danach in Zotero mit Suchpfad, Dokumenttyp und thematischer Vorzuordnung getaggt und in den in Abschnitt~\hyperref[sec:Systematische-Literaturrecherche]{4.2.1} beschriebenen Screening- und Taggingprozess überführt [@doring_forschungsmethoden_2023, Kapitel 10.6; @galvan_writing_2025].
