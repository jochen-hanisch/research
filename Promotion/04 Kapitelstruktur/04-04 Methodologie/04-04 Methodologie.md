...
In einem ersten Entscheidungsschritt wird die Verfügbarkeit der Quelle geprüft (Zugriff auf Volltext, Abstract, bibliografische Angaben). Ist der Volltext nicht zugänglich, kann die Quelle dennoch als Impulsgeber für die Suchstrategie dienen, wird aber nicht in die inhaltliche Hauptanalyse übernommen. Verfügbare Quellen aus der explorativen Spur werden daraufhin geprüft, ob sich aus ihnen konkrete Suchbegriffe ableiten lassen. Ergibt sich ein inhaltlicher Mehrwert, werden sie genutzt, um das Suchvokabular zu verfeinern und zusätzliche, thematisch angrenzende Begriffskombinationen zu identifizieren.
...
Aus dem Screening geht kein formal gleichgewichteter, sondern ein gestufter Arbeitskorpus hervor. Höher gewichtet werden solche Quellen, die einen direkten Bezug zum LMS- oder Bildungsraumkontext aufweisen, methodisch hinreichend erschlossen sind und für eine oder mehrere Forschungsunterfragen einen eigenständigen theoretischen oder empirischen Beitrag leisten. Randständige, stark formatspezifische oder nur indirekt anschlussfähige Titel bleiben als Kontext- und Sensibilisierungsmaterial sichtbar, tragen aber die spätere Verdichtung nicht in gleicher Weise. Die Gewichtung folgt damit keinem statistischen Aggregationsmodell, sondern einer dokumentierten Relevanz- und Anschlusslogik, wie sie für narrative und konzeptionelle Reviews angemessen ist [@galvan_writing_2025; @doring_forschungsmethoden_2023].
...
Die Suchordnerlogik ist entlang primärer, sekundärer und tertiärer Suchbegriffe aufgebaut. Damit werden Kernbezüge, kontextualisierende Nachbarschaften und periphere Trendfelder unterschieden. Die Ordnerkennungen (z. B. `S:01`) entsprechen der Zotero-Struktur und wurden zugleich als Tags verwendet, sodass Suchpfad und spätere Auswertung durchgehend aufeinander bezogen bleiben.
...
Sekundäre Begriffe erweitern den Blick auf didaktische und organisatorische Kontexte. Sie erfassen hybride Arrangements, mediale Settings und bildungstechnologische Konzepte, in die das LMS funktional eingebettet ist. Die Sichtungsquote liegt hier bei 50 %, weil diese Ebene vor allem der Kontextualisierung und der Identifikation flankierender Mechanismen dient, nicht aber der unmittelbaren Bestimmung des Kernkorpus [@doring_forschungsmethoden_2023, Kapitel 10.6; @galvan_writing_2025].
...
Alle folgenden Visualisierungen sind als deskriptive Korpusdiagnostik zu lesen (\hyperref[fig:summary-suchergebnisse]{Abb.~\ref{fig:summary-suchergebnisse}}) und strukturieren Verteilungen und Relationen (Kopplungen, Häufigkeiten, Flusspfade); sind gleoichzeitig keine Kausalmodelle und enthalten keine normativen Bewertungen. Interpretationen werden daher konsequent als Rückbindung an die Forschungsunterfragen und an die deduktiven Kategorien geführt (Abschnitt \hyperref[sec:Datenanalyse]{4.3} und Anhang A).
...
2. **Item‑Level statt Blackbox‑Index:** Primäre Auswertung pro Item mit ergänzendem Blick auf aggregierte Skalen, um FU‑Bezüge und konkrete Gestaltungsdimensionen nachvollziehbar zu halten.
...
Klar positive Paare zeigen sich zwischen technologischer Integration, Bewertungsmethoden und kollaborativem Lernen einerseits sowie Suchbegriffen wie `digital learning`, `E‑Learning` und `blended learning` andererseits. Negative Werte treten vor allem bei spezifischen Medientiteln auf, bleiben aber schwach. Für die methodische Einordnung spricht dies dafür, dass die Suchbegriffe den indexbasierten Schwerpunkten folgen und zwischen technologie- beziehungsweise didaktiknahen und peripheren Termfeldern differenzieren.
...
Für den Vergleich wurden im Eye‑Tracking pro Jahrgang jeweils acht Personen berücksichtigt ($n=24$ insgesamt); damit liegen für alle drei Kohorten gleich große, auf AOI-Ebene vergleichbare Teilstichproben vor, auch wenn die Repräsentativität zwischen den Jahrgängen unterschiedlich ausfällt [@hanisch-johannsen_wirkgefuge_2025-1]. Inhaltlich konzentriert sich die Auswertung auf die Stimuli `F10-S3` (Lernplan/Kompetenzen; Verständlichkeit), `F11-S3` (weiterführende Quellen; Auffindbarkeit und Link-Salienz) und `F14-S3` (Lernmaterial/Sicherheit; Salienz sicherheitsrelevanter Hinweise) sowie auf die zugehörigen jahrgangsbezogenen Gesamt-Visuals aus Heatmap, Viewmap und Fog‑View [@hanisch-johannsen_wirkgefuge_2025-1]. Die drei Stimuli wurden gewählt, weil sie unterschiedliche, für das LMS zentrale Funktionsbereiche abdecken: Orientierung im Kompetenz- und Lernverlauf, Sichtbarkeit weiterführender Angebote und Wahrnehmung sicherheitsrelevanter Materialien. Damit wird die Triangulation an drei methodisch unterschiedlichen, aber funktional komplementären Interfacesituationen durchgeführt, ohne den gesamten Bildkorpus im Detail erneut berichten zu müssen.
