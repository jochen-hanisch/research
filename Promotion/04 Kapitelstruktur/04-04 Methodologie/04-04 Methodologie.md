...
Der gesamte Prozess – von Beginn (Google-Alert beziehungsweise zufällige Quelle) über Screening, doppeltes Tagging und LLM-Unterstützung bis hin zu Netzwerk- und Pfadanalysen – ist damit als zirkulärer, transparent dokumentierter Recherche- und Analysepfad angelegt. Er stellt sicher, dass die in dieser Arbeit entwickelten Aussagen zu digitalen Bildungsräumen nicht auf Einzelstudien, sondern auf einem systematisch erschlossenen und strukturell ausgewerteten Literaturfeld beruhen.
...
Für die Methodologie ist daran vor allem eines wichtig: Die Suchordner sind nicht nur ein technisches Ablagesystem, sondern der erste operative Schritt der forschungsfragengeleiteten Korpusbildung. Sie entscheiden mit darüber, welche Literatur mit welcher Priorität in die Analyse gelangt, und bilden damit die Brücke zur Korpusdiagnostik in Abschnitt~\hyperref[sec:Visualisierungen-Literaturbasis]{4.2.3}, zur Auswertung in Abschnitt~\hyperref[sec:Datenanalyse]{4.3} und zur späteren FU-bezogenen Triangulation. Gerade deshalb gehört die Suchordnerlogik zur methodischen Nachvollziehbarkeit des gesamten Designs: Sie macht den Übergang von einer offenen Recherche zu einem kontrolliert aufgebauten, analysierbaren Literaturfeld explizit.
...
- Modellgestützte Bildauswertung: Falls multimodale Modelle zur Bildbeschreibung genutzt werden, dienen sie ausschließlich als Assistenz (kein automatisiertes Urteil); Modell/Version wird dokumentiert (#todo (#60, 2025-12-19)), und alle Interpretationen werden manuell gegengeprüft (COPE/DFG‑konform).
...
2. **Promptbasierte Auswertung:** Ein LLM-gestütztes Textanalysewerkzeug (GPT) erzeugt strukturierte Markdown-Analysen, die deduktiv definierte Kategorien ausfüllen und mit Originalzitaten aus der Quelle verknüpfen.
...
- LLM-gestützte Label-Vorschläge, die durch die Forschende überprüft und theoretisch validiert werden.
- Ableitung konsistenter Kodiermanuale mitsamt Ankerbeispielen.

Die eingesetzten Modelle (Embedding-Modell für Vektorrepräsentationen; LLM für Labeling/Redaktion) wirken als strukturierende Werkzeuge; Steuerung und Interpretation liegen vollständig bei der Forschenden.
...
6. **Ableitung und Revision der Kategorien (Mayring + P-QIA)** – LLM-gestützte Label, theoretische Validierung, Kodierleitfaden (Definitionen, Ankerbeispiele, Kodierregeln), Kodiermanual [@baur_qualitative_2022, Seite 696].
...
- Kombination klassischer Methoden (Literatur, Simulation, Eye-Tracking [@hanisch-johannsen_wirkgefuge_2025-1]) mit innovativen Ansätzen (LLM/Embeddings, Python).
...
- Limitationen:
  ...
  - Abhängigkeit von LLM-/Embedding-Tools und Simulationen.
