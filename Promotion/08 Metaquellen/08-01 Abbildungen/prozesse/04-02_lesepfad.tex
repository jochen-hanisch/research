...
\begin{tikzpicture}[node distance=2.3cm and 2cm, on grid, auto,
  every node/.style={rectangle, draw, align=center, minimum height=1.4cm, text width=3.5cm},
...
% Rückwärtsschleifen für explorativen Pfad
\draw[->, dashed, bend left=45] (modell) to (beduerfnisse);
\draw[->, dashed, bend left=35] (modell) to (bildungswiss);
\draw[->, dashed, bend left=25] (modell) to (medien);
...
\caption{Strukturierte Lektürepfade durch Kapitel 2 – linear und explorativ.}
\end{figure}
