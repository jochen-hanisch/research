...
created: 11.03.2025 | [updated](https://github.com/jochen-hanisch/research): 12.03.2025 | [published]():  | [Austausch](https://github.com/jochen-hanisch/research/discussions) | [[Hinweise]]
...
- Wie Unsicherheitsrelationen $\Delta K_L$, $\Delta E_L$, $\Delta N_L$ Kompetenzsprünge auslösen.  
...
graph TD;
    ...
    C -->|Unsicherheitsrelation einführen| D["Unsicherheitsrelation: C = ΔK_L * ΔE_L + w_1 * ΔK + w_2 * ΔE + w_3 * ΔN_L"];
...
- Kompetenzmessunsicherheit $\Delta K_L$  
- Kompetenzentwicklungsunsicherheit $\Delta E_L$  
- Emotionale Unsicherheit $\Delta E$  
- Kognitive Unsicherheit $\Delta K$  
- Neugierunsicherheit $\Delta N_L$  
...
$$  
C = \Delta K_L \cdot \Delta E_L + w_1 \cdot \Delta K + w_2 \cdot \Delta E + w_3 \cdot \Delta N_L  
\tag{2.5}  
$$  
...
2. Der Kompetenzzuwachs $\Delta K$ pro Zeitschritt hängt von der aktuellen Unsicherheit $C(t)$ ab und folgt der Beziehung  
...
3. Berechnung des Kompetenzzuwachses für jeden Zeitschritt anhand der Funktion  
   ...
   $$  
   K(t+1) = K(t) + \Delta K, \quad \text{wenn } C_{\text{eff}} > 0  
   \tag{2.19}  
   $$  
...
5. **Simulationsergebnisse als Validierung des Modells**  
   Monte-Carlo-Simulationen bestätigen, dass Kompetenzsprünge genau dann auftreten, wenn die Neugierunsicherheit $ \Delta N_L $ in kritische Wertebereiche gelangt.  
